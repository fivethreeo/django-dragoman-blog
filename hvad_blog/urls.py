...
urlpatterns = patterns('',
    ...
    url(r'^(?P<year>\d{4})/$', YearArchiveView.as_view(), name='hvad_blog_archive_year'),
    
    url(r'^(?P<year>\d{4})/(?P<month>\d{2})/$',
        MonthArchiveView.as_view(), name='hvad_blog_archive_month'),
    
    url(r'^(?P<year>\d{4})/(?P<month>\d{2})/(?P<day>\d{2})/$',
        DayArchiveView.as_view(), name='hvad_blog_archive_day'),
